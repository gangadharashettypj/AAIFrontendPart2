...
const AvatarView = () => {
  ...
  return (
    <div id="container">
      ...
      <div id="buttons">
        <input type="radio" id="chatOption" name="option" value="chat" defaultChecked />
        <label htmlFor="chatOption" className="button" title="agentManager.chat() -> Communicate with your Agent (D-ID LLM)">
          Chat
        </label>
        <input type="radio" id="speakOption" name="option" value="speak" />
        <label htmlFor="speakOption" className="button" title="agentManager.speak() -> Streaming API (Bring your own LLM)">
          Speak
        </label>
      </div>
      <div className="inputsDiv">
        <div className="mainInput">
          <Textarea id="textArea" placeholder="Write something — ‘Chat’ replies, ‘Speak’ repeats." autoFocus />
          <button className="roundButton" id="speechButton" title="Speech to Text - Web Speech API (MDN)"></button>
          <Button id="actionButton" className="roundButton" title="Send Message">
      ...
      <details id="answers">
        <summary>
          {' '}
          Chat History{' '}
          <svg style={{ verticalAlign: 'text-top' }} xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none">
            <path
              fill="currentcolor"
            ...
            />
            <path stroke="currentcolor" strokeLinecap="round" strokeWidth="1.5" d="M8 9h8M8 12.5h5.5" />
          </svg>
        </summary>
      </details>
    </div>
